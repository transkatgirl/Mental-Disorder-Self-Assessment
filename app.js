	...
	for (const [key, value] of scores.entries()) {
		if (key in assessment.scales) {
			...
			if (assessment.scales[key].scoring.percentile.function == "normal") {
				const z_score = (score - assessment.scales[key].scoring.percentile.mean) / assessment.scales[key].scoring.percentile.stddev;

				if (assessment.scales[key].scoring.percentile.show_t_score) {
					score = (z_score * 10) + 50;
				}
				...
				percentile = (Math.log(score) * assessment.scales[key].scoring.percentile.multiplier) + assessment.scales[key].scoring.percentile.intercept;
			}
